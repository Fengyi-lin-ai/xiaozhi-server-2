            ...
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    if res.optional.event == EVENT_SessionCanceled:
                        ...
                        # 修改: 确保添加到opus_datas_wav的是bytes类型数据
                        # if isinstance(res.payload, bytes):
                        #     opus_datas_wav.append(res.payload)
                        # elif isinstance(res.payload, (list, tuple)):
                        #     # 如果是列表，将其转换为bytes
                        #     opus_datas_wav.append(bytes(res.payload))
                        opus_datas = self.wav_to_opus_data_audio_raw(res.payload)
                        logger.bind(tag=TAG).debug(
                            f"推送数据到队列里面帧数～～{len(opus_datas)}"
                        )
                        opus_datas_cache.extend(opus_datas)
                        if len(opus_datas_cache) >= 10:
                            self.tts_audio_queue.put(
                        ...
                        # if opus_datas_wav:
                        #     filename = str(uuid.uuid4())+f"测试音频.wav"
                        #     with open(filename, "wb") as f:
                        #         # 修改: 处理opus_datas_wav中的数据，确保都是bytes类型
                        #         bytes_data = []
                        #         for data in opus_datas_wav:
                        #             if isinstance(data, bytes):
                        #                 bytes_data.append(data)
                        #             elif isinstance(data, (list, tuple)):
                        #                 bytes_data.append(bytes(data))
                        #             else:
                        #                 bytes_data.append(bytes([data]))
                        #         f.write(b''.join(bytes_data))
